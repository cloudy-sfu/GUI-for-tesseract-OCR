...
class BatchOCR(QThread):
    ...
    def run(self) -> None:
        ...
        for parent, _, files in os.walk(self.input_folder):
            filenames += [(parent, x) for x in files if os.path.splitext(x)[1].lower()
                          in ['.jpg', '.png']]
        ...
        for i, (parent, name) in zip(range(n_files), filenames):
            ...
            try:
                text = tesserocr.file_to_text(filepath, oem=tesserocr.OEM.TESSERACT_ONLY,
                                              path='raw/models', lang=self.lang)
            except Exception as e:
                self.gui_message.emit(f'{filepath} - {e}')
                self.gui_message.emit('Maybe you have not imported a language model. '
                                      'Please follow the usage at '
                                      'https://github.com/cloudy-sfu/GUI-for-tesseract-OCR '
                                      'and import your first language model.')
            ...
            try:
                ...
                self.gui_message.emit(f'{filepath} - Unicode encode error.')
                self.gui_message.emit("The image contains characters that cannot be "
                                      "encoded by the character set of your operation "
                                      f"system, {locale.getencoding()}.")
                self.gui_message.emit("Please try recognizing this file in \"Single "
                                      "File\" mode.")
            del text
            progress_percentage = int((i + 1) / n_files * 100)
            self.progress.emit(progress_percentage)
        self.gui_message.emit('Click "File | Open target folder" in the menu bar to view '
                              'the exported images.')
        self.done.emit(True)
...
class SingleOCR(QThread):
    ...
    def run(self) -> None:
        try:
            text = tesserocr.file_to_text(self.input_file,
                                          oem=tesserocr.OEM.TESSERACT_ONLY,
                                          path='raw/models', lang=self.lang)
        except Exception as e:
            self.gui_message.emit(f'{self.input_file} - {e}')
            self.gui_message.emit('Maybe you have not imported a language model. Please '
                                  'follow the usage at '
                                  'https://github.com/cloudy-sfu/GUI-for-tesseract-OCR '
                                  'and import your first language model.')
            self.done.emit(True)
...
class ClipboardOCR(QThread):
    ...
    def run(self) -> None:
        try:
            text = tesserocr.image_to_text(self.pillow_image,
                                           oem=tesserocr.OEM.TESSERACT_ONLY,
                                           path='raw/models', lang=self.lang)
        except Exception as e:
